...
public class ResourceList extends JTabbedPane
	...
	public ResourceList() {
		try {
			...
			Main.IconMap.put("sample", nullImage);
		} catch (IOException e) {
		}
	...
	public void write() {
		...
		String[] keySet = Main.IconMap.keySet().toArray(new String[Main.IconMap.keySet().size()]);
		Arrays.sort(keySet);
		for (String name : keySet) {
			iconModel.addElement(name);
		}
		soundModel.removeAllElements();
		keySet = Main.SoundMap.keySet().toArray(new String[Main.SoundMap.keySet().size()]);
		Arrays.sort(keySet);
		for (String name : keySet) {
			soundModel.addElement(name);
		}
		scopeModel.removeAllElements();
		keySet = Main.ScopeMap.keySet().toArray(new String[Main.ScopeMap.keySet().size()]);
