...
public class Recoil extends DataBase {
	...
	/** 横反動の基礎値 +で右 Curve */
	public static final DataEntry<Curve> HorizontalBase = of(new Curve(), new Info().Scale("0,1").KeyMin(0).KeyMax(1));
	/** 横反動の拡散値 +で右 Curve */
	public static final DataEntry<Curve> HorizontalSpread = of(new Curve(),
			new Info().Scale("0,1").Min(0).KeyMin(0).KeyMax(1));
	/** 横反動の復帰率 射撃したタイミングで戻りの予約を入れる Curve */
	public static final DataEntry<Curve> HorizontalReturn = of(new Curve(),
			new Info().Scale("0,1").Min(0).KeyMin(0).KeyMax(1));
	/** 横反動適応時間 Curve */
	public static final DataEntry<Curve> HorizontalRecoilTick = of(new Curve(), new Info().Min(0).KeyMin(0).KeyMax(1));
	/** 横反動復帰時間 Curve */
	...
	public static final DataEntry<Curve> VerticalReturn = of(new Curve(),
			new Info().Scale("0,1").Min(0).KeyMin(0).KeyMax(1));
	/** 縦反動適応時間 Curve */
	public static final DataEntry<Curve> VerticalRecoilTick = of(new Curve(), new Info().Min(0).KeyMin(0).KeyMax(1));
	/** 縦反動復帰時間 Curve */
