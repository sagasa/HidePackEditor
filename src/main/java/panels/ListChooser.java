...
public class ListChooser extends JPanel implements ComponentListener, MouseListener, ActionListener {
	...
	void write() {
		...
		case GUN_MAGAZINE_LIST:
			...
			for (BulletData data : Window.BulletList.values()) {
				combo.addItem(data.ITEM_INFO.NAME_SHORT);
			}
			listModel.clear();
			for (String str : (String[]) ValueInfo.getData(Data, GunDataList.BULLET_USE)) {
		...
		case GUN_FireMode:
			...
			listModel.clear();
			for (String str : (String[]) ValueInfo.getData(Data, GunDataList.FIREMODE)) {
				combo.removeItem(str);
				listModel.addElement(new ListComponent(str));
			}
			break;
	...
	void delete(int index) {
		ValueInfo.setData(Data, getDataType(), helper.ArrayEditer.RemoveFromArray(
				(String[]) ValueInfo.getData(Data, getDataType()), listModel.getElementAt(index).name));

		write();
	}

	/** EnumDataList取得 */
	private EnumDataInfo getDataType() {
		switch (mode) {
		case GUN_MAGAZINE_LIST:
	...
	public void mousePressed(MouseEvent e) {
		...
		if (!listModel.isEmpty()) {
			...
			Point offsetPoint = new Point(e.getX() - model.indexToLocation(index).x,
					e.getY() - model.indexToLocation(index).y);
			if (comp.isInDeleteBotton(offsetPoint)) {
				delete(model.locationToIndex(e.getPoint()));
			} else if (comp.isInUpBotton(offsetPoint)) {
				// 上のインデックスと置き換え
				ValueInfo.setData(Data, getDataType(), helper.ArrayEditer
						.ChangeArrayIndex((String[]) ValueInfo.getData(Data, getDataType()), index, index - 1));
				write();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (combo.getSelectedIndex() != -1) {
			ValueInfo.setData(Data, getDataType(), helper.ArrayEditer
					.AddToArray((String[]) ValueInfo.getData(Data, getDataType()), combo.getSelectedItem().toString()));
			write();
		}
	...
	class ListComponent extends JPanel implements ListCellRenderer<ListComponent> {
		...
		// レンダーメゾット
		@Override
		public Component getListCellRendererComponent(JList<? extends ListComponent> list, ListComponent value,
